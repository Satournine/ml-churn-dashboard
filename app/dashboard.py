...
def user_input_form():
    ...
    PhoneService = st.sidebar.selectbox(
        "Phone Service", ["Yes", "No"], key="PhoneService"
    )
    MultipleLines = st.sidebar.selectbox(
        "Multiple Lines", ["No", "Yes", "No phone service"], key="MultipleLines"
    )
    ...
    OnlineSecurity = st.sidebar.selectbox(
        "Online Security", ["Yes", "No", "No internet service"], key="OnlineSecurity"
    )
    OnlineBackup = st.sidebar.selectbox(
        "Online Backup", ["Yes", "No", "No internet service"], key="OnlineBackup"
    )
    ...
    TechSupport = st.sidebar.selectbox(
        "Tech Support", ["Yes", "No", "No internet service"], key="TechSupport"
    )
    StreamingTV = st.sidebar.selectbox(
        "Streaming TV", ["Yes", "No", "No internet service"], key="StreamingTV"
    )
    StreamingMovies = st.sidebar.selectbox(
        "Streaming Movies", ["Yes", "No", "No internet service"], key="StreamingMovies"
    )
    ...
    MonthlyCharges = st.sidebar.number_input(
        "Monthly Charges", 0.0, 150.0, 70.0, key="MonthlyCharges"
    )
    TotalCharges = st.sidebar.number_input(
        "Total Charges", 0.0, 10000.0, 2500.0, key="TotalCharges"
    )
...
with tab2:
    ...
    with col1:
        st.image(
            "plots/confusion_xgb_tuned.png",
            caption="Confusion Matrix",
            use_container_width=True,
        )
    with col2:
        st.image(
            "plots/roc_xgb_tuned.png", caption="ROC Curve", use_container_width=True
        )

with tab3:
    st.header("Top Features Driving Churn")
    st.image(
        "plots/xgb_importance.png",
